...
- **A waiting row says what it wants.** When a dispatcher stops, the row quotes the question its last message closed on — "Want me to merge it?", "Say the word and I'll…" — taken whole from the `Stop` hook, not the first line of the transcript. `r` answers it in one line without attaching; a permission prompt, which is a menu, still takes `enter`.
- **An API error is not a stall.** A turn an API error ended used to fire nothing, so the row said *working* over a session sitting idle. The `StopFailure` hook reports it now, and a transient one (overloaded, server error) is retried with `continue` on a 1m/5m/15m backoff from the cockpit's poll — the row says `api error · overloaded · retry 1 of 3 in 40s` meanwhile. A usage limit, credential or billing error is yours, and the row says which. Existing installs re-run `init` to get the hook.
- **Every dispatch closes with its mode's working contract** — how far to take the work, to wait on CI or a deploy with a background task or `Monitor` rather than ending the turn, to take the next step the brief implies instead of offering it, and to stop only for a decision that is yours, ending on that one question.
- **A steward can run the fleet for you.** Press `t` on the triage table to switch it on (or off); the headline says `steward on`. It is a Claude Code session of the dispatcher's own, kept running by the cockpit for as long as it is switched on — through reboots and restarts — and you never have to look at it: what it does shows on your rows. It sleeps on `status --next` as a background task — costing nothing while the fleet is quiet — and wakes when a dispatcher is waiting and nobody has acted on it. What the brief already settles ("want me to write the tests next?", "shall I carry on?") it answers with a one-line `steward:` reply; anything leaving the branch — a merge, a deploy, a message, a deletion — and anything outside the brief it leaves for you with a note of the facts: the row reads `steward · yours: merge #12 — CI green, no review, touches billing`. It cannot edit files, merge or push. Its own session keeps a running account of the fleet if you want to read it (`: steward` jumps in); `claude-dispatcher steward` / `steward stop` flip the same switch from a shell.
- **The fleet is scriptable.** `claude-dispatcher status [--json]` prints the triage table's reading — every live dispatcher, most urgent first, with its ask; `reply`, `park` and `unpark` are its hand-acts. They exist so a Claude Code session can steward the fleet.
...
| `r` | reply — type one line into a waiting dispatcher's session, without attaching |
| `t` | steward on/off — it answers what each brief settles and notes the rest on your rows |
...
- **A dispatcher says *working* but has not moved, and its transcript ends on `API Error`** — the `StopFailure` hook entry is newer than your install. Re-run `claude-dispatcher init` to add it.
