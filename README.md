...
- **Fan-outs are visible.** When a session spins out subagents, the same hooks report each one starting and stopping, and the dispatcher's row says so beside its CI signal — `fan-out · 3 live` while they run, `fanned out 12 subagents` once the turn is home, with the detail panel naming the agent types. Installed with the other hooks; existing installs re-run `init` to get it.
